/**
 * Server-side input normalisation for Choccy Chat submissions.
 *
 * IMPORTANT: keep these helpers in lock-step with src/lib/countries.ts and
 * src/lib/instagram.ts in the Next.js side. The Worker is a separate npm
 * package, so we duplicate rather than cross-import.
 *
 * The country list here is just (canonicalName, alias[]) pairs — we don't
 * carry display metadata because the Worker only validates input. The
 * frontend has the richer dataset.
 */

const IG_URL_RE = /^(https?:\/\/)?(www\.|m\.)?instagram\.com\//i;
...
/* ─── Country list ────────────────────────────────────────────────────────
   Mirror of COUNTRIES in src/lib/countries.ts. */
type CountrySpec = { name: string; code: string; aliases?: string[] };
...
/** True if `value` is a finite latitude in [-90, 90]. */
export function isValidLat(value: unknown): value is number {
  return typeof value === "number" && Number.isFinite(value) && value >= -90 && value <= 90;
}

/** True if `value` is a finite longitude in [-180, 180]. */
