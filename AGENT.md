...
- **Adding a static sub-route under a path that already has a `[id]` dynamic route.** Every `[id]` route has a catch-all SPA rewrite in `public/_redirects` (`/production/:id → /production/_spa/`). A `:id` placeholder matches *any* non-empty segment, including a literal like `"new"`, so `/production/new` silently gets rewritten to the `/production/_spa/` shell on Cloudflare while serving the correct static file locally. The symptom: page loads on localhost, but on Cloudflare it hydrates the wrong HTML and throws React error #418. **Fix:** add explicit pass-through rules for the literal sub-path *before* the catch-all in `public/_redirects` — see the `/production/new/*` block there for the pattern. Do this at the same time as creating the new page, not as a follow-up. **This guard is Cloudflare Pages-specific.** The pass-through works because Pages resolves a self-referential `200` rule (`/production/new/ → /production/new/ 200`) to "serve the asset at this path". Vercel needs no guard at all — it checks the filesystem before applying `vercel.json` rewrites, which is why there is no `/production/new` entry there. Cloudflare **Workers** static assets, however, follow redirects *"regardless of whether or not an asset matches the incoming request"*, so the self-referential rule does not win and the request falls through to the `:id` catch-all — the exact bug this guard exists to prevent, silently reintroduced. Pointing the rule at `index.html` instead loops, because Workers' default `html_handling: "auto-trailing-slash"` 307s `/foo/index.html` back to `/foo/`; `html_handling: "none"` is the escape hatch. We deploy on Pages and Vercel, so this is not currently a live problem — but treat `public/_redirects` as Pages semantics, not portable config. Reported in [#168](https://github.com/choc-collab/app/issues/168).
- **Budget: `_redirects` allows only 100 *dynamic* rules** (those containing `:name` or `*`) on both Pages and Workers, against a 2,000 cap for literal ones. Each `[id]` route costs three. We are currently at 71, so roughly nine more dynamic routes before rules start being dropped — and the failure is silent. Check with `awk '!/^\s*#/ && NF>=2 && $1 ~ /[:*]/' public/_redirects | wc -l` before adding a batch of routes.
...
- **`src/app/(public)/getting-started/page.tsx`** — end-user Getting Started guide (22-section walkthrough). Screenshots referenced by this guide live in `public/docs/screenshots/` and are regenerated by `npm run docs:screenshots` (Playwright script at `e2e/docs-screenshots.spec.ts`).
...
LogEntry       id, date (local ISO "YYYY-MM-DD"; indexed), body, createdAt, updatedAt
               — a free-text note in the daily Log; several per day. The day's automatic
               summary is NOT stored: `computeDigestIndex()` in lib/dailyLog derives it live.

LogDay         id, date (local ISO; indexed, not unique — readers take the latest updatedAt),
               ambientTempC?, humidityPct?, updatedAt — day-level workshop conditions
...
- Tables: `ingredients`, `products`, `productCategories`, `fillings`, `fillingCategories`, `ingredientCategories`, `productFillings`, `fillingIngredients`, `moulds`, `productionPlans`, `planProducts`, `planStepStatus`, `settings` (legacy, kept for backup compat), `userPreferences`, `productFillingHistory`, `ingredientPriceHistory`, `coatingChocolateMappings`, `productCostSnapshots`, `experiments`, `experimentIngredients`, `packaging`, `packagingOrders`, `shoppingItems`, `collections`, `collectionProducts`, `collectionPackagings`, `collectionPricingSnapshots`, `decorationMaterials`, `decorationCategories`, `shellDesigns`, `fillingStock`, `labelTemplates`, `sales`, `giveaways`, `orders`, `customers`, `orderProductionLinks`, `orderLineItems`, `logEntries`, `logDays` (schema v20)
...
Log: `useLogEntries()` (by date), `useLogEntriesForDay(date)`, `addLogEntry(date, body)`, `updateLogEntryFields(id, changes, description)` (autosave), `deleteLogEntry(id)`, `useLogDays()`, `useLogDay(date)`, `saveLogDayConditions(date, {ambientTempC, humidityPct})` (upsert by date), `useLogSources()` — one live subscription over every table the digest reads (plans, plan products/fillings/step statuses, moulds, products sans photo, fillings, filling stock, sales, give-aways, orders, customers, experiments, ingredients, price history, packaging + orders, shopping items) plus the currency symbol; `undefined` until first resolve.
...
## Daily Log (`src/lib/dailyLog.ts`)
Pure derivation for the Log. The automatic day summary ("digest") is never stored — it is computed from the other tables so it back-fills history and follows edits. Nothing calls `new Date()`; days are local ISO strings.
- `isoDayOf(value)` — local ISO day of a Date / ms / ISO string (ISO dates pass through; garbage → null). `shiftISODate(iso, delta)`, `formatLongDate(iso)` ("Wednesday 9 September 2026"), `formatShortDate(iso)` ("Wed 9 Sep"), `relativeDayLabel(iso, todayISO)` ("Today" / "Yesterday" / null).
- `computeDigestIndex(sources)` → `Map<iso, DayDigest>` — one pass over every table in `LogSources`; each `DigestLine` has an `area` (workshop | shop | orders | lab | pantry), `text`, optional `detail` and `href`, and a `rank` for ordering. Production steps are read from `planStepStatus.doneAt` via `parseStepKey(key, planProductIds, planFillingById)` (matches UUIDs against known ids rather than splitting on "-"; decoration `*-after-*` steps ignored) and aggregated per day + plan + phase, counting each plan product once ("Coloured 6 moulds"). `computeDayDigest(iso, sources)`, `digestHeadline(digest, max)`, `emptyDigest(iso)`.
- Known gaps (no timestamp to read): order status changes, experiment promotion, edits to recipes. Filling stock made by a plan is covered by the step line, so only hand-recorded stock rows appear.
- Entries: `entriesByDate(entries)`, `logDayByDate(days)` (latest `updatedAt` wins on duplicates), `formatConditions(day)` ("21°C · 55% RH"), `logDaysForList(digests, entries, days, {todayISO, period, notesOnly, search})` → newest-first `LogDaySummary[]` (days after `todayISO` are never listed; the calendar likewise renders no markers for future days), `groupLogDaysByMonth(days)`, `parseConditionValue(raw)`.
...
src/
  app/
    ...
    (public)/               — public, unauthenticated marketing + docs
      ...
      getting-started/
        page.tsx            — 22-section end-user reference hub: four grouped card grids (Get set up, Build your pantry, Run the workshop, Labels/backup/reference); clicking a card opens the full section in a modal with prev/next navigation; Esc closes. Uses <Shot src="/docs/screenshots/…png" /> for real captures.
    ...
    log/
      page.tsx              — the Log: table of days grouped by month (LogTable: activity headline, notes, conditions; period / notes-only filters, search) or a month calendar with per-area markers; list⇄calendar toggle persisted
      [date]/page.tsx       — one day (`/log/2026-09-09`): autosaving notes first (several per day, two-step delete), then the "What happened" digest with links back; sidebar: workshop conditions, per-area tally; previous/next day (no next beyond today)
    collections/
  ...
  components/
    orders/
      ...
      month-grid.tsx        — order chips on the shared MonthGridShell
    log/
      area-style.tsx        — AREA_INK per Log area + AreaDot
      digest-list.tsx       — DigestList (grouped by area, links back) + DigestPreview (compact, for cards/tile)
      log-table.tsx         — LogTable: month-grouped table of days on the PantryTable chrome (day, what happened, notes, conditions)
      log-month-grid.tsx    — Log calendar: per-area count markers + note pencil per day, on MonthGridShell
      log-notes.tsx         — LogNotes (autosaving note cards, two-step delete) + AddNoteBox (also used by the Today tile)
      conditions-card.tsx   — workshop temperature / humidity sidebar card (save on blur / Enter)
    today/log-tile.tsx      — "Today's log" dashboard panel: digest preview + quick note box
    list-calendar-toggle.tsx — shared List ⇄ Calendar segmented toggle (Orders, Log)
    month-grid-shell.tsx    — month calendar chrome (heading, prev/Today/next, weekday row, 7-col cells); callers render cell bodies
  ...
  lib/
    ...
    categoryDelete.ts       — pure `decideCategoryDelete()`: hard-delete or archive-only? Filling/ingredient categories link by name, so the rule asks whether the NAME survives, which is what makes duplicate rows removable (unit-tested; imports no `db`)
...
| `src/lib/dailyLog.test.ts` | Date helpers (`isoDayOf`, `shiftISODate`, `formatLongDate`, `formatShortDate`, `relativeDayLabel`), `parseStepKey` (slot suffixes, standalone plan fillings, decoration steps ignored), `computeDigestIndex` per area (step aggregation counting each plan product once, pieces via actualYield vs cavities, finished batches, freezer moves, hand-recorded filling stock, sales prepared/sold with revenue, give-aways, orders + multi-day events with cancelled skipped, customers, experiments/forks, pantry lines, area/rank ordering, name truncation), `computeDayDigest`, `digestHeadline`, `entriesByDate`, `logDayByDate`, `formatConditions`, `logDaysForList` (union, period, notes-only, search), `groupLogDaysByMonth`, `parseConditionValue` |
...
| `e2e/log.spec.ts` | Log: empty state + nav entry; + opens today; day page notes (add, autosave edit surviving reload, two-step delete, several per day, Ctrl/Cmd+Enter); workshop conditions save and show in the table row; previous/next day (none beyond today) and unparseable day; automatic digest from an order created today (creation line links back; future event day not listed); table grouped by month with collapse, search, click-through; calendar markers, month navigation, persisted view; Today tile quick note |
